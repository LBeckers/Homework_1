...
```{r challenge1_sol, echo=FALSE, out.width="50%"}
cdc_males <- read_csv("data/CDC_Males.csv")

correl_cdc <- cdc_males %>% 
  filter(type.fac == "Firearm-related") %>% 
  summarise(cor(adjusted.suicide.White, adjusted.homicide.White))

correl_cdc

cdc_males %>% 
  filter(type.fac == "Firearm-related") %>% 
  ggplot(aes(x = adjusted.suicide.White, y = adjusted.homicide.White, colour = gun.house.prev.category)) +
  geom_point(aes(size = average.pop.white)) + 
  scale_size_area(breaks = c(500000, 1500000, 3000000, 7000000), 
                  labels = c('500k', '1.5m', '3m', '7m'), max_size = 14) +
  scale_color_manual(values = c("10.2%-19.9%" = "#fef0d9", "20.0%-34.9%" = "#fdcc8a", "35.0%-44.9%" = "#fc8d59", "45.0%-65.5%" = "#d7301f")) +
